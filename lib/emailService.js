export async function sendAssessmentEmail({ parentName, childName, grade, subject, loginId, parentEmail }) {
  ...
  const html = `
    <div style="font-family:'Helvetica Neue',Arial,sans-serif;max-width:600px;margin:0 auto;background:#ffffff;">
      <div style="background:#1a2744;padding:24px 32px;border-radius:12px 12px 0 0;">
        <div style="color:white;font-family:Georgia,serif;font-size:20px;font-weight:700;">TutorKlub</div>
        <div style="color:#6de4d8;font-size:12px;margin-top:4px;">We teach the way your child learns.</div>
      </div>
      <div style="padding:32px;background:#faf8f4;">
        ...
        <p style="font-size:15px;color:#1a2744;font-weight:700;margin:0 0 8px;">
          ${childName}'s Grade ${grade} ${subject} Diagnostic Assessment is now available.
        </p>
        ...
        <div style="background:#ffffff;border-radius:12px;padding:24px;margin-bottom:24px;border:1px solid #e0ddd6;">
          ...
          <p style="font-size:14px;color:#5a6478;margin:0 0 8px;">
            <strong style="color:#1a2744;">Login ID:</strong>
            <span style="font-family:monospace;background:#f0fdf4;color:#0e8a7c;padding:2px 8px;border-radius:4px;font-weight:700;">${loginId}</span>
          </p>
          <p style="font-size:14px;color:#5a6478;margin:0 0 8px;">
            <strong style="color:#1a2744;">Assessment Link:</strong>
            <a href="${assessmentLink}" style="color:#0e8a7c;">${assessmentLink}</a>
          </p>
          <p style="font-size:14px;color:#5a6478;margin:0;">
            <strong style="color:#1a2744;">Duration:</strong> Approximately 45 minutes
          </p>
        </div>
      ...
      <div style="background:#1a2744;padding:16px 32px;border-radius:0 0 12px 12px;text-align:center;">
        <p style="font-size:12px;color:rgba(255,255,255,0.4);margin:0;">© 2026 TutorKlub · hello@tutorklub.com</p>
      </div>
    </div>
  `;
  ...
  const response = await fetch("/api/send-email", {
    ...
    body: JSON.stringify({ to: parentEmail, subject: subjectLine, html }),
  });
