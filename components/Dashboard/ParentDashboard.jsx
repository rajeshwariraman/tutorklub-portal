...
const C = {
  ...
  amber:"#d97706", muted:"#5a6478", soft:"#8491a8",
};
    ...
    <div style={{textAlign:"center"}}>
      <svg width={128} height={128} viewBox="0 0 128 128">
        ...
        <text x={cx} y={cy-5} textAnchor="middle" fontSize={20}
          fontWeight={700} fill={C.navy} fontFamily="Georgia,serif">
          {score}/{total}
        </text>
        <text x={cx} y={cy+13} textAnchor="middle" fontSize={10}
          fill={C.soft} fontFamily="Nunito,sans-serif">Score</text>
      </svg>
      <div style={{fontWeight:700,fontSize:"0.82rem",color:col,marginTop:2}}>
        {lbl}
      </div>
    </div>
  );
  ...
  return(
    <div style={{
      ...
      <div style={{
        ...
        fontWeight:700,color:color||C.navy,lineHeight:1
      }}>{val}</div>
      <div style={{fontSize:"0.7rem",color:C.soft,fontWeight:600,marginTop:"0.3rem"}}>
        {label}
      </div>
...
export default function ParentDashboard(){
  ...
  const studentQs=student
    ...
    :[];
  const score=student
    ?studentQs.reduce((s,q,i)=>s+(student.answers[i]===q.a?1:0),0)
    :0;
  const topicData=student
    ?studentQs.map((q,i)=>({
        ...q,
        correct:student.answers[i]===q.a,
        selectedOpt:q.opts[student.answers[i]],
  ...
  <div style={{maxWidth:1160,margin:"0 auto",padding:"2rem 1.5rem"}}>
    ...
    <div style={{textAlign:"center",marginBottom:"20px"}}>
      <button
        onClick={()=>window.location.href=
          "https://calendly.com/tutorklub/free-assessment"}
        style={{
          background:"#0e8a7c",color:"white",
    ...
    {!student&&(
    <div style={{
      background:C.white,borderRadius:18,padding:"2.5rem",
      ...
      }}>Open Student Results</h2>
      <p style={{
        fontSize:"0.85rem",color:C.muted,
        marginBottom:"1.5rem",lineHeight:1.7
      }}>
        Enter the student's Login ID to view their full assessment dashboard.
        Share results with parents during the demo session.
      </p>
      <input
        value={loginInput}
      ...
      <p style={{fontSize:"0.72rem",color:"#aaa",marginTop:"0.75rem"}}>
        Demo: type TK-AR4821 and press Enter
      </p>
    </div>)}
    ...
    <div style={{
      ...
      <h2 style={{
        fontFamily:"Georgia,serif",fontSize:"1.45rem",
        color:C.navy,marginBottom:"0.5rem"
      }}>Results Ready: {student.childName}</h2>
      <p style={{
        fontSize:"0.85rem",color:C.muted,
        marginBottom:"1.5rem",lineHeight:1.7
      }}>
        Present to <strong>{student.parentName}</strong> when you
        are in the demo session. Click below to reveal all results.
      </p>
      <div style={{
        ...
        {[
          ...
          <div key={k} style={{
            ...
            padding:"0.35rem 0",borderBottom:"1px solid #f0ede8"
          }}>
            <span style={{
              fontSize:"0.78rem",color:C.soft,fontWeight:600
            }}>{k}</span>
            <span style={{
              fontSize:"0.78rem",color:C.navy,fontWeight:700,
              fontFamily:k==="Login ID"?"monospace":"inherit"
            }}>{v}</span>
          </div>
        ))}
      </div>
      <button
        ...
        style={{
          width:"100%",background:C.gold,color:C.white,
          border:"none",borderRadius:50,padding:"0.9rem",
          fontSize:"0.9rem",fontWeight:700,cursor:"pointer",
          boxShadow:"0 4px 14px rgba(201,146,42,0.3)"
        }}>
        Reveal Results to Parent →
      </button>
      <button
        onClick={()=>{setStudent(null);setLoginInput("");}}
        style={{
          width:"100%",background:"transparent",color:"#aaa",
          border:"1px solid #e0ddd6",borderRadius:50,
    ...
    <>
      <div style={{
        ...
        <div>
          <p style={{
            ...
            color:"#6de4d8",margin:"0 0 0.2rem"
          }}>Assessment Results — Demo Session</p>
          <h1 style={{
            fontFamily:"Georgia,serif",fontSize:"1.65rem",
            color:C.white,margin:0
          }}>{student.childName}</h1>
          <p style={{
            color:"rgba(255,255,255,0.5)",
            fontSize:"0.8rem",margin:"0.25rem 0 0"
          }}>
            Grade {student.grade} · {student.subject} · {student.date}
          </p>
        </div>
      ...
      <div style={{
        ...
        <StatCard
          icon="📋" val={studentQs.length}
          label="Questions" sub="Total answered"/>
        <StatCard
          icon="✅" val={score}
          label="Correct" color={C.green}/>
        <StatCard
          icon="📌" val={studentQs.length-score}
          label="Need Attention" color={C.red}/>
        <StatCard
          icon="📊"
          val={Math.round(score/studentQs.length*100)+"%"}
          label="Overall Score"
          color={
            score/studentQs.length>=0.8?C.green:
